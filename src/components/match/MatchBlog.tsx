...
const Card = ({ children, className = '' }: { children: React.ReactNode; className?: string }) => (
  <div className={`rounded-2xl border border-white/[0.06] p-5 md:p-6 space-y-4 ${className}`}
    ...
    <div className="space-y-5 text-sm text-gray-300 leading-relaxed">
      ...
      <Card>
        ...
        {broadcastInfo && (
          <div className="flex items-center gap-2 pt-3 border-t border-white/[0.06]">
            <span className="text-gray-600 text-xs">📺 Watch on:</span>
          ...
          <Card key={team.name}>
            ...
            <div className="flex items-center gap-3">
              {team.logo ? (
                ...
                <div className="w-8 h-8 rounded-full bg-red-600/20 border border-red-500/30 flex items-center justify-center text-white text-xs font-black">
                  {team.name[0]}
                </div>
              )}
      ...
      <Card>
        ...
        <div className="grid grid-cols-3 gap-3 text-center">
          <div className="rounded-xl p-4 border border-green-500/20" style={{ background: 'rgba(34,197,94,0.05)' }}>
            <p className="text-green-400 text-3xl font-black">{h2h.homeWins}</p>
            <p className="text-gray-500 text-xs mt-1">{home.name} wins</p>
          </div>
          <div className="rounded-xl p-4 border border-yellow-500/20" style={{ background: 'rgba(234,179,8,0.05)' }}>
      ...
      <div className="rounded-2xl p-5 md:p-6 space-y-4 border border-red-500/20"
        style={{ background: 'linear-gradient(135deg, rgba(127,29,29,0.3) 0%, #111 100%)' }}>
